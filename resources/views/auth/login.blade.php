...
<div class="app-grid flex min-h-screen items-center justify-center px-6 py-12">
  <div class="glass w-full max-w-md rounded-3xl p-8">
    ...
    <form method="POST" action="{{ route('otp.send') }}">
      ...
      <div class="mb-2 flex overflow-hidden rounded-xl border border-line bg-panel-soft"><span class="flex items-center border-r border-line px-4 font-mono text-sm text-zinc-400">+233</span><input name="phone" inputmode="tel" autocomplete="tel" required class="w-full bg-transparent px-4 py-3.5 text-sm outline-none" placeholder="24 000 0000"></div>
      @error('phone')<p class="mb-4 text-xs text-rose-400">{{ $message }}</p>@enderror
      <button class="mt-3 w-full rounded-xl bg-pink py-3.5 text-sm font-extrabold transition hover:bg-pink-soft">Send verification code</button>
    </form>
